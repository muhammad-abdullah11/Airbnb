...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const hasFilters = searchParams.get('location') || searchParams.get('guests') || searchParams.get('checkIn');

    let url = '/api/v1/listings/active';
    if (hasFilters) {
      url = `/api/v1/listings/search${location.search}`;
    }
  ...
  if (listings.length === 0) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center space-y-4">
        <p className="text-gray-600 text-lg">No listings match your search criteria</p>
        <button
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h2 className="text-2xl font-semibold text-gray-900 mb-6">
          {location.search ? `Search results (${listings.length})` : 'Popular homes →'}
        </h2>
